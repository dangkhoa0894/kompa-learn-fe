...
import AxesChartView from 'components/common/Chart/Am/AxesChart';
  ...
  const renderChart = () => {
    switch (typeModel) {
      ...
      case 2: {
        ...
        if (tempScore.algorithm?.coreId < 6 || tempScore?.algorithm?.coreId === 10) {
          dataChart = [
            {
              category: 'Recall',
        ...
        return (
          <>
            ...
            <div className="analysis-tag">
              {tempScore?.algorithm?.coreId < 6 || tempScore?.algorithm?.coreId === 10 ? (
                <BarChart
                  dataChart={dataChart}
                  showTitle={false}
                  id="chart_tags"
